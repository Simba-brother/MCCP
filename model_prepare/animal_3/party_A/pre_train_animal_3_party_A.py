'''
准备预训练模型 
refcode: https://www.kaggle.com/code/enisahovi/fastai-image-classification-cats-family
'''
import pandas as pd
import os
import sys
...
def start_train():
    # 加载数据
    train_csv_path = "/data/mml/overlap_v2_datasets/animal_3/party_A/dataset_split/train.csv"
    val_csv_path = "/data/mml/overlap_v2_datasets/animal_3/party_A/dataset_split/val.csv"
    ...
    # 获得batches
    classes = getClasses("/data/mml/overlap_v2_datasets/animal_3/party_A/dataset_split/train")
    ...
    base_model = load_model("/data/mml/overlap_v2_datasets/animal_3/party_A/models/standard_base_model/base_model_EfficientNetB3_224_224.h5")
    ...
    # 保存搭建好的模型结构
    model.save("/data/mml/overlap_v2_datasets/animal_3/party_A/models/model_struct/EfficientNetB3_224_224.h5")
    # 设置检查点
    saved_dir_path = "/data/mml/overlap_v2_datasets/animal_3/party_A/models/model_weights"
    checkpointer = ModelCheckpoint(os.path.join(saved_dir_path, 'model_weight_{epoch:03d}_{val_accuracy:.4f}.h5'),
                                    ...
                                    save_best_only=True)
    # 设置学习率减小
    learning_rate_reduction = ReduceLROnPlateau(monitor='val_accuracy', patience = 5, verbose=1, factor=0.6, min_lr=0.000001)
